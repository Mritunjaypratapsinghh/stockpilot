...
async def get_user_ledger(
    user_id: PydanticObjectId, type_filter: str = None, status: str = None, person: str = None, recurring: bool = None
):
...
async def add_settlement(ledger_id: PydanticObjectId, user_id: PydanticObjectId, amount: float, note: str = None):
    ...
    settlement = Settlement(user_id=user_id, amount=amount, date=datetime.now(timezone.utc), note=note)
