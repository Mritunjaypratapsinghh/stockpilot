...
async def scrape_ipo_data() -> None:
    ...
    old_ipos = await IPO.find(In(IPO.status, ["OPEN", "UPCOMING"])).to_list()
    ...
    try:
        async with httpx.AsyncClient(timeout=15) as client:
            resp = await client.get(
                "https://ipowatch.in/ipo-grey-market-premium-latest-ipo-gmp/", headers={"User-Agent": "Mozilla/5.0"}
            )
                    ...
                    for row in rows[:20]:
                        ...
                        if len(cols) >= 6:
                            ...
                            lot_size = (
                                ...
                                else max(1, int(15000 / price)) if price > 0 else 1
                            )
...
async def check_ipo_alerts() -> None:
    ...
    relevant_ipos = [
        ...
        if ipo.dates
        and (
            (ipo.dates.get("allotment") and ipo.dates["allotment"].date() == today)
            or (ipo.dates.get("listing") and ipo.dates["listing"].date() == today)
        )
    ]
    ...
    users = await User.find(User.settings.alerts_enabled == True).to_list()  # noqa: E712
