    ...
    <div className="min-h-screen bg-[var(--bg-primary)] flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm">
          ...
          <p className="text-center text-sm text-[var(--text-muted)] mt-8">{isLogin ? "Don't have an account? " : 'Have an account? '}<button onClick={() => { setIsLogin(!isLogin); setError(''); }} className="text-[var(--accent)] hover:text-[#5558e3] font-medium">{isLogin ? 'Sign up' : 'Sign in'}</button></p>
        </div>
      </div>
    </div>
  );
