...
export default function AlertsPage() {
  ...
  const loadAlerts = () => getAlerts().then(setAlerts);
  useEffect(() => { loadAlerts().finally(() => setLoading(false)); }, []);

  const [createAlert, creating] = useAsyncAction(
    async (e) => {
      e.preventDefault();
      await api('/api/alerts', { method: 'POST', body: JSON.stringify({ symbol: symbol.toUpperCase(), alert_type: alertType, target_value: parseFloat(targetValue) }) });
      setSymbol(''); setTargetValue(''); setShowForm(false);
      loadAlerts();
    },
    { successMsg: 'Alert created' }
  );

  const [deleteAlert, deleting] = useAsyncAction(
    async (id) => {
        ...
        {showForm && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={() => setShowForm(false)}>
            <div className="w-full max-w-md bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg p-6" onClick={e => e.stopPropagation()}>
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6"><h2 className="text-lg font-semibold">Create Alert</h2><button onClick={() => setShowForm(false)} className="p-2 text-[var(--text-muted)] hover:text-white rounded-lg hover:bg-[var(--border)]"><X className="w-5 h-5" /></button></div>
              <form onSubmit={createAlert} className="space-y-4">
                ...
                <button type="submit" disabled={creating} className={`w-full py-3 bg-[var(--accent)] text-white rounded-lg font-medium hover:bg-[#5558e3] ${creating ? 'opacity-50' : ''}`}>{creating ? 'Creating...' : 'Create'}</button>
              </form>
            </div>
          </div>
        )}
