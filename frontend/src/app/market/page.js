    ...
    <div className="min-h-screen bg-[var(--bg-primary)]">
      ...
      <main className="p-4 md:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <h1 className="text-xl md:text-2xl font-bold">Market Overview</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          {loading ? [...Array(3)].map((_, i) => <div key={i} className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg p-5 h-28 animate-pulse" />) : Object.entries(indices).map(([name, data]) => (
            <div key={name} className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-lg p-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-2"><span className="text-[var(--text-muted)]">{name}</span>{data.change_pct >= 0 ? <TrendingUp className="w-5 h-5 text-[#10b981]" /> : <TrendingDown className="w-5 h-5 text-[#ef4444]" />}</div>
            ...
            {fiiDii?.note ? <div className="p-8 text-center text-[var(--text-muted)]">{fiiDii.note}</div> : (
              <div className="p-5 space-y-4">
                <div className="bg-[var(--bg-primary)] rounded-lg p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3"><span className="font-medium">FII (Foreign)</span><span className={`text-xs px-2 py-1 rounded ${(fiiDii?.fii?.net || 0) >= 0 ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-[#ef4444]/10 text-[#ef4444]'}`}>{(fiiDii?.fii?.net || 0) >= 0 ? 'Net Buyer' : 'Net Seller'}</span></div>
                ...
                <div className="bg-[var(--bg-primary)] rounded-lg p-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3"><span className="font-medium">DII (Domestic)</span><span className={`text-xs px-2 py-1 rounded ${(fiiDii?.dii?.net || 0) >= 0 ? 'bg-[#10b981]/10 text-[#10b981]' : 'bg-[#ef4444]/10 text-[#ef4444]'}`}>{(fiiDii?.dii?.net || 0) >= 0 ? 'Net Buyer' : 'Net Seller'}</span></div>
